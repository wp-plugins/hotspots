 ...
 */
function enqueueHotSpotScripts() {
...
add_action( 'wp_enqueue_scripts', 'enqueueHotSpotScripts' );


/**
 * Enqueue admin hotspots scripts and setup local variables
 */
function enqueueAdminHotSpotScripts() {

	wp_enqueue_script('jquery');
	wp_enqueue_script('hotspot-admin', plugins_url('hotspot-admin.js', __FILE__), array('jquery'));
...
add_action( 'admin_enqueue_scripts', 'enqueueAdminHotSpotScripts' );
...
function createHotSpotDBTable() {
	...
	$wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}hotspots");
	$sql1 = "CREATE TABLE {$wpdb->prefix}hotspots (
...
function addMouseClick() {
	...
	if (wp_verify_nonce($ajaxNonce, 'hotspot-nonce')) {
		...
		$rows_affected = $wpdb->insert( "{$wpdb->prefix}hotspots", array( 'x' => $x, 'y' => $y ) );
	}
    die(); 
...
function getMouseClicks() {
	...
	if (wp_verify_nonce($ajaxNonce, 'hotspot-nonce')) {
		...
		$rows = $wpdb->get_results( "SELECT x, y FROM {$wpdb->prefix}hotspots" );
	}
...
function hotSpotOptions() {
	?>
	<div id="hotSpotOptions" class="wrap meta-box-holder">
	<?php 
	// enabled option
	...
	if (isset($_POST['hotValue'])) {
		if (is_numeric($_POST['hotValue'])) {
			...
			if ($hotValue > 0) {
				...
				echo "<p class='errorMessage'>Hot value must be numeric greater than 0.</p>";
			}
		} else {
			echo "<p class='errorMessage'>Hot value must be numeric greater than 0.</p>";
		}
	...
	if (isset($_POST['spotOpacity'])) {
		if (is_numeric($_POST['spotOpacity'])) {
			...
			if ($spotOpacity >= 0 && $spotOpacity <= 1) {
				...
				echo "<p class='errorMessage'>Spot opacity must be numeric between 0 and 1.</p>";
			}
		} else {
			echo "<p class='errorMessage'>Spot opacity must be numeric between 0 and 1.</p>";
		}
	...
	if (isset($_POST['spotRadius'])) {
		if (is_numeric($_POST['spotRadius'])) {		
			...
			if ($spotRadius >= 1 && $spotRadius <= 25) {
				...
				echo "<p class='errorMessage'>Spot radius must be numeric between 1 and 25.</p>";
			}
		} else {
			echo "<p class='errorMessage'>Spot radius must be numeric between 1 and 25.</p>";
		}
	...
	?>
		...
		<form method="post" action="#" id="hotSpotsOptionsForm">
			<div class="optionRow">
				<input type="checkbox"
					value="<?php echo $default_enabled ?>" name="enabled" id="enabled"
					<?php if ($default_enabled == "on") { ?> checked="checked" <?php } ?> />
			...
			<div class="optionRow">
				<input
					type="checkbox" value="<?php echo $default_showOnClick ?>"
					name="showOnClick" id="showOnClick"
					<?php if ($default_showOnClick == "on") { ?> checked="checked"
					<?php } ?> />
			...
			<div class="optionRow">
				...
				<input type="text"
					value="<?php echo $default_hotValue ?>" name="hotValue"
					id="hotValue" />
			</div>
			...
			<div class="optionRow">
				...
				<input type="text"
					value="<?php echo $default_spotRadius ?>" name="spotRadius"
					id="spotRadius" />
			</div>
			...
			<div class="optionRow">
				...
				<input type="text"
					value="<?php echo $default_spotOpacity ?>" name="spotOpacity"
					id="spotOpacity" />
			</div>
			
			<div class="optionRow">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes'); ?>" />
				<button id="refreshBtn" class="button">Refresh Database</button>
			</div>
...
function hotspotsAdminStyle() {
	...
	<style type="text/css">
		...
		div#hotSpotOptions div.optionRow { margin-bottom: 20px; }
		div#hotSpotOptions p.errorMessage { color: Red; font-style: italic; display: inline-block; margin-right: 5px;}
	</style>
	';
}
 ...
 */
function refreshHotSpotsDBTable() {
	global $wpdb;

	$ajaxNonce = $_POST['nonce'];
	$rows = null;
	if (wp_verify_nonce($ajaxNonce, 'hotspot-nonce')) {
		$rows = $wpdb->get_results( "DELETE FROM {$wpdb->prefix}hotspot WHERE 1" );
	}
